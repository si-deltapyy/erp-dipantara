...
test.skip(process.env.E2E_PRODUCTION === 'true', 'Kayu mock navigation')
test('confirms dirty browser navigation and keeps a small-screen form keyboard accessible', async ({
    ...
    await page.getByRole('link', { name: 'Master Kayu', exact: true }).click()
    await page.getByRole('button', { name: 'Tambah Kayu', exact: true }).click()
    await page.getByLabel('Nama Kayu', { exact: true }).fill('Draft Navigasi')
    ...
    await expect(page.getByLabel('Nama Kayu', { exact: true })).toHaveValue('Draft Navigasi')
    ...
    await expect(page.getByRole('dialog', { name: 'Tambah Kayu', exact: true })).toHaveCount(0)
