...
test('upgrades version two without deleting existing data and rolls back failed Mitra writes', async ({
    page,
}) => {
    await page.goto('/app')
    const result = await page.evaluate(async () => {
        ...
        const name = 'mitra-upgrade-' + crypto.randomUUID()
        ...
        const before = await runDemoTransaction(
            options,
            ['mitras', 'buyers', 'buyerMutations', 'audit', 'samples', 'blobs'],
            'readonly',
            async (transaction) => ({
                count: await transaction.count('mitras'),
        ...
        await runDemoTransaction(
            options,
            ['mitras', 'metadata', 'mitraMutations', 'audit'],
            'readwrite',
            async (transaction) => {
                await transaction.clear('mitras')
